<template>
  <div class="leftWenyu">
    <ul class="leftwenyu-ul">
      <li class="leftwenyu-ul-li" v-for="(item, index) in data" :key="item.id">
        <!-- 头像和用户名 -->
        ...
        <!-- 图片 -->
        <div class="wenyu-body">
          <div class="wenyu-body-p">{{ item.bodyP }}</div>
          <div class="wenyu-body-pictures">
            <div class="row" v-for="item2 in item.bodyPictures" :key="item2.id">
              <div
                class="wenyu-body-pictures-num"
...
export default {
  ...
  methods: {
    ...
    postComments(e, id) {
      ...
      if (obj !== '{}') {
        ...
        if (inputValue === '') {
          ...
          let commentslength = ul.children.length + 1; // 获取评论区的长度
          if (commentslength === 0) {
          ...
          // 把评论过来的数据存到vuex里
          this.$store.commit(
            'MAINMIDDLEWENYU/getPostCommentsData',
            postCommentsObj
          );
          this.$nextTick(() => {
            // 发完评论 清空input值
    ...
    removeComments(e, id, commentsId) {
      ...
      if (obj !== '{}') {
        ...
        this.$store.commit('MAINMIDDLEWENYU/removeComments', ids);
  ...
  computed: {
    ...mapState({ data: (state) => state.MAINMIDDLEWENYU.data }),
    ...mapState({
      commentsDataList: (state) => state.MAINMIDDLEWENYU.commentsDataList
    })
  },
  created() {
